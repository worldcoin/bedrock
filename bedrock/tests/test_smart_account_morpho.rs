...
use bedrock::{
    ...
    transactions::contracts::{
        constants::WLD_TOKEN_ADDRESS,
        morpho::{Morpho, MorphoToken, MORPHO_VAULT_ADDRESS},
    },
};
...
#[tokio::test]
async fn test_morpho_deposit_wld() -> anyhow::Result<()> {
    ...
    let morpho_vault = IMorphoVault::new(MORPHO_VAULT_ADDRESS, &provider);
    ...
    // Execute the deposit via the Is4337Encodable trait
    ...
    let after_safe_wld = wld.balanceOf(safe_address).call().await?;
    let after_vault_shares = morpho_vault.balanceOf(safe_address).call().await?;
