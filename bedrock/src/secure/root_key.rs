...
/// Parses the provided Ethereum secret key into an Ethereum wallet `LocalSigner<SigningKey>` for use with `alloy`.
///
/// It decodes the key from a hex string into bytes and parses it into a `LocalSigner<SigningKey>`.
///
/// # Errors
...
impl RootKey {
    ...
    pub fn identity(&self) -> crate::identity::Identity {
        crate::identity::Identity::new(
            self.worldid_key().unwrap(),
            crate::identity_context::IdentityContext::Orb,
        )
        .as_ref()
        .clone()
    }

    pub fn personal_custody_keypair(&self) -> crate::PersonalCustodyKeypair {
        self.orb_encryption_key().unwrap().as_ref().clone()
    }
}
