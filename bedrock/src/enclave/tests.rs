...
fn generate_fake_attestation_expired_cert() -> Vec<u8> {
    ...
    let expired_timestamp = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_millis() as u64 - (365 * 24 * 60 * 60 * 1000); // 1 year ago
...
fn test_attestation_with_different_root_ca() {
    ...
    assert!(result.is_err(), "Should reject attestation signed by different root CA");
    ...
    match result {
        ...
        Err(other) => {
            println!("✅ Rejected attestation with different root CA (error: {:?})", other);
        }
        Ok(_) => {
