...
| J | Bedrock `flows/{remove_factor,delete_backup,reset,logout}.rs`; native reset/logout/account-delete callsites. | C, E, F, I | Unbound reset works without a signer and deletes Turnkey through break-glass; missing break-glass returns incomplete cleanup; retry never deletes a live/new suborg; logout clears local state after remote timeout/auth failure, tries both stores, preserves backup/other factors; Busy/pending recovery blocks teardown; local deletion errors surface and retry unbound; missing/unusable signer still clears local state and reports incomplete revocation; signer reference released; imported-key/local-reset paths make no remote calls. |
...
During C, native callers bind their root and stored signer once, then supply only the root,
...
| `BedrockBackupManager.swift`, `BedrockBackupServiceApi.swift`, `WLDBedrock/BedrockWrapper.swift` | One manager instance, startup bind with root/signer, head check, and callback registration; remove the native backup-service callback once C/F cover its callers. |
...
| `AuthorizeDeviceReducer` | Persist a fresh signer, bind it for the same account, then call reauthorize; handle TurnkeyStatus. No root transfer, files, vault import, or wallet login. |
...
| `AppReducer`, `AccountDeletion`, `KeyManagementService` | Await logout before erasing native keys/data; preserve keys on Busy/RecoveryPending; report remote/local cleanup failures; use logout(false) for intentional local reset; retain existing keychain-login access. |
...
| `RestoreBackupViaPasskey`, `RestoreBackupViaGoogleOAuth`, `RunTurnkeyMigrations` | Shared recovery/reauth contract; secure root handoff; persist and bind pending signer before recovery; remove blank Turnkey-user-ID sentinel. |
...
| `BedrockSdk`, `BedrockBackupServiceApi`, `OxideBedrockBridgeImpl` | Register one manager and `MainFactorCeremony` adapter; bind the root/signer and check the head at startup; batch file mutations; remove the native sync/metadata callback and old manifest methods. |
