...
| J | Bedrock `flows/{remove_factor,delete_backup,reset,logout}.rs`; native reset/logout/account-delete callsites. | C, E, F, I | Reset deletes Turnkey through break-glass; missing break-glass returns incomplete cleanup; retry never deletes a live/new suborg; logout clears local state after remote timeout/auth failure, tries both stores, preserves backup/other factors; Busy/pending recovery blocks teardown; local deletion errors surface and retry unbound; signer-load failure is reported; imported-key/local-reset paths make no remote calls. |
