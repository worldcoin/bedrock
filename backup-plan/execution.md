...
| `BedrockSdk`, `BedrockBackupServiceApi`, `OxideBedrockBridgeImpl` | Register one manager and `MainFactorCeremony` adapter; initialize with the root/signer and check the head at startup; batch file mutations; remove the native sync/metadata callback and old manifest methods. |
